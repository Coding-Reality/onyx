...
def test_allowed_tenant_ids_are_independent_of_public_hosts() -> None:
    assert load_allowed_tenant_ids(json.dumps([TENANT_A, TENANT_B])) == frozenset(
        {TENANT_A, TENANT_B}
    )
...
async def _request(
    ...
    middleware = TenantContextMiddleware(
        app, middleware.tenant_host_map, middleware.allowed_tenant_ids
    )
    middleware._session_tenant = AsyncMock(return_value=session_tenant)
...
async def test_host_mapping_sets_and_clears_tenant_context() -> None:
    ...
    middleware = TenantContextMiddleware(
        lambda: None, {"a.example.com": TENANT_A}, frozenset({TENANT_A})
    )
...
@pytest.mark.asyncio
async def test_allowed_credential_tenant_overrides_bootstrap_tenant() -> None:
    middleware = TenantContextMiddleware(
        lambda: None,
        {"a.example.com": TENANT_A},
        frozenset({TENANT_A, TENANT_B}),
    )
    ...
    assert status == 200
    assert observed_tenant == TENANT_B


@pytest.mark.asyncio
async def test_session_tenant_overrides_bootstrap_tenant() -> None:
    middleware = TenantContextMiddleware(
        lambda: None,
        {"onyx.example.com": TENANT_A},
        frozenset({TENANT_A, TENANT_B}),
    )
    status, observed_tenant = await _request(
        middleware, "onyx.example.com", session_tenant=TENANT_B
    )
    assert status == 200
    assert observed_tenant == TENANT_B
...
@pytest.mark.asyncio
async def test_session_and_credential_tenants_must_agree() -> None:
    middleware = TenantContextMiddleware(
        lambda: None,
        {"onyx.example.com": TENANT_A},
        frozenset({TENANT_A, TENANT_B}),
    )
    authorization = f"Bearer on_{TENANT_A}.not-a-real-secret"
    status, observed_tenant = await _request(
        middleware,
        "onyx.example.com",
        authorization,
